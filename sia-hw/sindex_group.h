...
class alignas(CACHELINE_SIZE) Group {
 ...
 public:
  ...
  float mean_error_est() const;
  float get_mean_error() const;
  Group *split_model();
  Group *merge_model();
 ...
 private:
  ...
  inline float train_model(size_t model_i, size_t begin, size_t end);

  inline std::vector<std::pair<key_t, size_t>> merge_refs(record_t *&new_data, uint32_t &new_array_size,
  ...
  inline void merge_refs_with(const Group &next_group, record_t *&new_data,
                              ...
                              int32_t &new_capacity) const;
  inline std::vector<std::pair<key_t, size_t>> merge_refs_internal(record_t *new_data,
  ...
  inline float *get_model(size_t model_i) const;
  ...
  inline void prepare_last(size_t model_i,
                           const std::vector<float *> &model_keys,
                           const std::vector<size_t> &positions);
  inline size_t get_error_bound(size_t model_i,
                                const std::vector<float *> &model_key_ptrs,
  ...
  inline size_t predict(size_t model_i, const float *model_key) const;
  inline bool key_less_than(const uint8_t *k1, const uint8_t *k2,
                            size_t len) const;

  inline bool incremental_training(uint32_t model_n, std::vector<std::pair<key_t, size_t>> delta_vector);
  inline bool incremental_training(uint32_t model_n, std::vector<std::pair<key_t, size_t>> delta_vector, size_t p_len, size_t f_len);
  ...
  std::array<uint8_t,
             (max_model_n - 1) * sizeof(key_t) +
                 max_model_n * sizeof(float) * (key_t::model_key_size() + 1)>
      model_info;
  
  // Cached matrix
  float *cached_r[max_model_n] = {nullptr};
};
