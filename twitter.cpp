...
inline void prepare_sindex(sindex_t *&table) {
  char filename[256];
  sprintf(filename, "/dataset/twitter/load%s", cluster_number);
...
void *run_fg(void *param) {
  ...
  // Read workload trace file of the thread
    char filename[256];
    sprintf(filename, "/dataset/twitter/%s/th_%02ld/workload_%02d", cluster_number, fg_n, thread_id);
...
inline void parse_args(int argc, char **argv) {
  struct option long_options[] = {
      ...
      {0, 0, 0, 0}};
  std::string ops = "a:b:c:d:e:f:g:h:i:j:k:l:m:n:o:p:q:r:w:z:";
  int option_index = 0;
