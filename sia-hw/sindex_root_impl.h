...
void Root<key_t, val_t, seq>::init(const std::vector<key_t> &keys,
  ...
  float max_group_error = 0, avg_group_error = 0;
  float avg_prefix_len = 0, avg_feature_len = 0;
  ...
  for (size_t group_i = 0; group_i < group_n; group_i++) {
    ...
    float err = get_group_ptr(group_i)->get_mean_error();
...
void Root<key_t, val_t, seq>::adjust_root_model() {
  ...
  std::vector<float> errors(group_n);
  for (size_t group_i = 0; group_i < group_n; group_i++) {
    errors[group_i] = std::abs(
        (float)group_i - (float)predict(get_group_ptr(group_i)->pivot) + 1);
  }
  float mean_error =
...
inline void Root<key_t, val_t, seq>::train_piecewise_model() {
  ...
  for (size_t m_i = 0; m_i < indexes.size(); ++m_i) {
    ...
    DEBUG_THIS("------ SIndex Root Model(" << m_i << "): size=" << m_size
                                           << ", p_len=" << p_len
                                           << ", f_len=" << f_len);
    std::vector<float> m_keys(m_size * f_len);
    std::vector<float *> m_key_ptrs(m_size);
    ...
    model_prepare(m_key_ptrs, ps, models[m_i].weights.data(), f_len, &root_cached_r[m_i]);
  }
}
...
inline void Root<key_t, val_t, seq>::grouping_by_partial_key(
  ...
  float group_error = 0;
  float avg_group_size = 0, avg_f_len = 0;
  ...
  std::vector<float> model_keys(keys.size() * sizeof(key_t));
...
template <class key_t, class val_t, bool seq>
inline float Root<key_t, val_t, seq>::train_and_get_err(
    std::vector<float> &model_keys, size_t start_i, size_t end_i, size_t p_len,
    size_t f_len) const {
  size_t key_n = end_i - start_i;
  assert(key_n > 2);
  std::vector<float *> model_key_ptrs(key_n);
  ...
  std::vector<float> weights(f_len + 1, 0);
  model_prepare(model_key_ptrs, positions, weights.data(), f_len, &root_cached_r[0]);
