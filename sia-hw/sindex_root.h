...
class Root {
  ...
  struct PartialModelMeta {
    uint32_t p_len;
    uint32_t f_len;
    std::array<float, key_t::model_key_size() + 1> weights;
  };
 ...
 private:
  ...
  inline size_t predict(const float *model_key);
  inline size_t pick_next_stage_model(size_t pos_pred);
  inline float *get_2nd_stage_model(const size_t model_i);
  ...
  inline float train_and_get_err(std::vector<float> &model_key, size_t start,
  ...
  float *root_cached_r[max_root_model_n] = {nullptr};
};
