...
void *run_fg(void *param) {
    ...
    while (running && token) {
        ...
        switch (op) {
            case 'g':
            {
                ct_kv *res = ct_lookup(table, MAX_KEY_SIZE, (uint8_t *)query_key.buf);
                break;
            }
            case 'p':
            {
                ...
                int res = 0;
                if (ct_lookup(table, MAX_KEY_SIZE, (uint8_t *)query_key.buf)) {
                    res = ct_update(table, kv);
                } else {
                    res = ct_insert(table, kv);
                }
                assert(res == S_OK);
                break;
            }
            ...
            case 's':
            {
                ...
                bool resume = false;
                do {
                    ct_iter_goto(iter, MAX_KEY_SIZE, (uint8_t *)query_key.buf);
