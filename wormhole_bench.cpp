...
inline void prepare_xindex(struct wormref *wh) {
    ...
    std::uniform_int_distribution<uint8_t> rand_int8(33, 126);
    ...
    for (size_t i = 0; i < initial_size; ++i) {
        ...
        auto res = whsafe_put(wh, kv_create((char *)k.buf, MAX_KEY_SIZE, "abcdefgh", 8));
        if (!res) {
            COUT_N_EXIT("init insert failed");
        }
    }
...
void *run_fg(void *param) {
    ...
    for (int i=0; i<LATEST_KEY_NUM; i++) {
        ...
        auto res = whsafe_put(wh, kv_create(op_keys[insert_i++].buf, MAX_KEY_SIZE, "a", 1));
    }
#endif
#ifdef HOTSPOT_DIST
    ...
    while (running) {
        ...
        if (d <= read_ratio) {  // get
            ...
            #ifdef SEQUENTIAL_DIST
                kref_ref_hash32(&kref, op_keys[(read_i + delete_i) % end_i].buf, MAX_KEY_SIZE);
                read_i++;
                if (unlikely(read_i == end_i)) read_i = 0;
            #endif
            #ifdef UNIFORM_DIST
                kref_ref_hash32(&kref, op_keys[p * read_i - 1].buf, MAX_KEY_SIZE);
            #endif
            #ifdef LATEST_DIST
                kref_ref_hash32(&kref, latest_keys[p * LATEST_KEY_NUM].buf, MAX_KEY_SIZE);
            #endif
            #ifdef HOTSPOT_DIST
                kref_ref_hash32(&kref, op_keys[hotspot_start + (hotspot_end - hotspot_start) * p].buf, MAX_KEY_SIZE);
            #endif
            #ifdef EXPONENT_DIST
                kref_ref_hash32(&kref, op_keys[e * read_i - 1].buf, MAX_KEY_SIZE);
            #endif
            #ifdef ZIPF_DIST
                kref_ref_hash32(&kref, op_keys[z * read_i - 1].buf, MAX_KEY_SIZE);
            #endif
            ...
            kv out;
            #ifdef SEQUENTIAL_DIST
                auto res = whsafe_put(wh, kv_create(op_keys[(update_i + delete_i) % end_i].buf, MAX_KEY_SIZE, "a", 1));
                update_i++;
                if (unlikely(update_i == end_i)) update_i = 0;
            #endif
            #ifdef UNIFORM_DIST
                auto res = whsafe_put(wh, kv_create(op_keys[p * insert_i - 1].buf, MAX_KEY_SIZE, "a", 1));
            #endif
            #ifdef LATEST_DIST
                auto res = whsafe_put(wh, kv_create(latest_keys[p * LATEST_KEY_NUM].buf, MAX_KEY_SIZE, "a", 1));
            #endif
            #ifdef HOTSPOT_DIST
                auto res = whsafe_put(wh, kv_create(op_keys[hotspot_start + (hotspot_end - hotspot_start) * p].buf, MAX_KEY_SIZE, "a", 1));
            #endif
            #ifdef EXPONENT_DIST
                auto res = whsafe_put(wh, kv_create(op_keys[e * insert_i - 1].buf, MAX_KEY_SIZE, "a", 1));
            #endif
            #ifdef ZIPF_DIST
                auto res = whsafe_put(wh, kv_create(op_keys[z * insert_i - 1].buf, MAX_KEY_SIZE, "a", 1));
            #endif
            ...
            #ifdef SEQUENTIAL_DIST
                kref_ref_hash32(&kref, op_keys[delete_i].buf, MAX_KEY_SIZE);
                delete_i++;
                if (unlikely(delete_i == end_i)) delete_i = 0;
            #endif
            #ifdef UNIFORM_DIST
                kref_ref_hash32(&kref, op_keys[p * insert_i].buf, MAX_KEY_SIZE);
            #endif
            #ifdef LATEST_DIST
                kref_ref_hash32(&kref, op_keys[p * insert_i].buf, MAX_KEY_SIZE);
            #endif
            ...
            #endif
            #ifdef EXPONENT_DIST
                kref_ref_hash32(&kref, op_keys[e * insert_i].buf, MAX_KEY_SIZE);
            #endif
            #ifdef ZIPF_DIST
                kref_ref_hash32(&kref, op_keys[z * insert_i].buf, MAX_KEY_SIZE);
            #endif

            auto res = whsafe_del(wh, &kref);
        } else {  // scan
            struct wormhole_iter * const iter = wormhole_iter_create(wh);
            struct kref kref;
...
void run_benchmark(struct wormref *wh, size_t sec) {
    ...
    while (current_sec < sec) {
        ...
        if ((insert_ratio != 0.0) && (total_keys >= target_size)) {
...
inline void parse_args(int argc, char **argv) {
    struct option long_options[] = {
        ...
        {0, 0, 0, 0}};
    std::string ops = "a:b:c:d:e:f:g:h:i:p:q:";
    int option_index = 0;
