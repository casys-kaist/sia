...
inline result_t Group<key_t, val_t, seq, max_model_n>::get(const key_t &key,
  ...
  res = get_from_array(key, val);
  clock_gettime(CLOCK_MONOTONIC, &end_t);
  interval = (end_t.tv_sec - begin_t.tv_sec) + (end_t.tv_nsec - begin_t.tv_nsec) / 1000000000.0;
  buf << "get_from_array: " << interval << std::endl;
  if (res) { 
    std::cout << buf.str();
    return result_t::ok;
  } 
  ...
  clock_gettime(CLOCK_MONOTONIC, &end_t);
  interval = (end_t.tv_sec - begin_t.tv_sec) + (end_t.tv_nsec - begin_t.tv_nsec) / 1000000000.0;
  buf << "get_from_buffer: " << interval << std::endl;
  if (res) { 
    std::cout << buf.str();
    return result_t::ok;
  } 
  ...
  if(buffer_temp) {
    clock_gettime(CLOCK_MONOTONIC, &begin_t);
    res = get_from_buffer(key, val, buffer_temp);
    clock_gettime(CLOCK_MONOTONIC, &end_t);
    interval = (end_t.tv_sec - begin_t.tv_sec) + (end_t.tv_nsec - begin_t.tv_nsec) / 1000000000.0;
    buf << "get_from_temp_buffer: " << interval << std::endl;
    if (res) { 
      std::cout << buf.str();
      return result_t::ok;
    } 
  ...
  //std::stringstream buf;
  //buf << "buffer_size: " << buffer->size() << std::endl;
  //std::cout << buf.str();
